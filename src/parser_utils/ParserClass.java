...
public class ParserClass {
    ...
    * @see #getOccurrences(String, int, String)
    ...
    public static Map<String,Integer> getOccurrencesFromFile(String filepath, int n, String regex) throws IOException {
        ...
        return getOccurrences(s.toString(),n,regex);
    }
    ...
    * @see #getOccurrences(String, int, String)
    ...
    public static String getBestOccurrencesFromFile(String filepath, int n, String regex, int k) throws IOException {
        return String.format("Article given : %s\n%s",filepath, getBestOccurrences(getOccurrencesFromFile(filepath, n, regex),k));
    }
    ...
    * Finds the most used pairs of words in every sentence of a file.
     */
    //TODO : from article string not filepath
    ...
    public static String getBestConcurrentPairsFromFile(String pathFile, int n, String regex, int k) throws IOException {
        return getBestConcurrentPairs(getConcurrentPairs(pathFile,n, regex), k);
    }
    ...
    /**
     * Same as {@link #getOccurrences(String, int, String)} but with synonyms taken in count (with {@link #getThesaurus()}).
     * @throws IOException In case the file cannot be read.
     */
    public static Map<String,Integer> getOccurrencesSynonyms(String filepath, String regex, int n) throws IOException {
        File file = new File(filepath);
        BufferedReader br = new BufferedReader(new FileReader(file));
    ...
    /*
    public static int getNumberOfOccurrences(Map<String,Integer> map){
        int sum=0;
        for(String key:map.keySet())
            sum+=map.get(key);
        return sum;
